...
public abstract class CommonFiniteProbabilityModelTests extends CommonProbabilityModelTests {
	
	//
	// PROTECTED
	//
	protected void test_RollingPairFairDiceModel_Distributions(FiniteProbabilityModel model, RandomVariable dice1RV, RandomVariable dice2RV) {
		...
		Assert.assertEquals(new double[] {1.0/6.0, 1.0/6.0, 1.0/6.0, 1.0/6.0, 1.0/6.0, 1.0/6.0}, dPriorDice1.getValues());
		
		Distribution dPriorDice2 = model.priorDistribution(dice2);
		Assert.assertEquals(6, dPriorDice2.getValues().length);
		Assert.assertEquals(new double[] {1.0/6.0, 1.0/6.0, 1.0/6.0, 1.0/6.0, 1.0/6.0, 1.0/6.0}, dPriorDice2.getValues());
		
		Distribution dPosteriorDice1GivenDice2 = model.posteriorDistribution(dice1, dice2);
		...
		for (int i = 0; i < dPosteriorDice1GivenDice2.getValues().length; i++) {
			Assert.assertEquals(1.0/6.0, dPosteriorDice1GivenDice2.getValues()[i], DELTA_THRESHOLD);
		}
		
		Distribution dPosteriorDice2GivenDice1 = model.posteriorDistribution(dice2, dice1);
		...
		for (int i = 0; i < dPosteriorDice2GivenDice1.getValues().length; i++) {
			Assert.assertEquals(1.0/6.0, dPosteriorDice2GivenDice1.getValues()[i], DELTA_THRESHOLD);
		}
		...
		for (int i = 0; i < dJointDice1Dice2.getValues().length; i++) {
			Assert.assertEquals(1.0/36.0, dJointDice1Dice2.getValues()[i], DELTA_THRESHOLD);
		}
		...
		for (int i = 0; i < dJointDice2Dice1.getValues().length; i++) {
			Assert.assertEquals(1.0/36.0, dJointDice2Dice1.getValues()[i], DELTA_THRESHOLD);
		}
		...
		// TODO - Determine how constraints across multiple variables should be represented in a
		// distribution. Currently going with {true, false} but it may make sense
		// to be all permutations of the random variables from the model that make up the constraint
		// - TBD.
		Assert.assertEquals(2, dPriorTotal11.getValues().length);
		Assert.assertEquals(2.0/36.0, dPriorTotal11.getValues()[0], DELTA_THRESHOLD);
		Assert.assertEquals(34.0/36.0, dPriorTotal11.getValues()[1], DELTA_THRESHOLD);
		
		IntegerSumProposition doubles = new IntegerSumProposition("Doubles", new FiniteIntegerDomain(2, 4, 6, 8, 10, 12), dice1RV, dice2RV);
		// TODO
	}
	
	protected void test_ToothacheCavityCatchModel_Distributions(FiniteProbabilityModel model, RandomVariable toothacheRV, RandomVariable cavityRV, RandomVariable catchRV) {
		Assert.fail("TODO");
