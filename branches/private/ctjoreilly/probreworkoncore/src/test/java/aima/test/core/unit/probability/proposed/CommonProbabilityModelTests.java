...
import aima.core.probability.proposed.proposition.DisjunctiveProposition;
import aima.core.probability.proposed.proposition.EquivalentProposition;
import aima.core.probability.proposed.proposition.IntegerSumProposition;
import aima.core.probability.proposed.proposition.SubsetProposition;
...
public abstract class CommonProbabilityModelTests {
	...
	protected void test_RollingPairFairDiceModel(ProbabilityModel model) {
		...
		Assert.assertEquals(1.0, model.prior(ExampleRV.DICE_2_RV),
				DELTA_THRESHOLD);
		Assert.assertEquals(1.0, model.posterior(ExampleRV.DICE_1_RV,
				ExampleRV.DICE_2_RV), DELTA_THRESHOLD);
		Assert.assertEquals(1.0, model.posterior(ExampleRV.DICE_2_RV,
				ExampleRV.DICE_1_RV), DELTA_THRESHOLD);
		...
		DisjunctiveProposition dice1Is5OrDice2Is5 = new DisjunctiveProposition(
				dice1Is5, dice2Is5);
		Assert.assertEquals(1.0 / 6.0 + 1.0 / 6.0 - 1.0 / 36.0, model
				.prior(dice1Is5OrDice2Is5), DELTA_THRESHOLD);
	}
	...
	protected void test_ToothacheCavityCatchModel(ProbabilityModel model) {
		...
		// P(b|a) = P(a|b)P(b)/P(a)
		Assert.assertEquals(model.posterior(acavity, atoothache), (model
				.posterior(atoothache, acavity) * model.prior(acavity))
				/ model.prior(atoothache), DELTA_THRESHOLD);
		Assert.assertEquals(model.posterior(acavity, anottoothache), (model
				.posterior(anottoothache, acavity) * model.prior(acavity))
				/ model.prior(anottoothache), DELTA_THRESHOLD);
		Assert.assertEquals(model.posterior(anotcavity, atoothache), (model
				.posterior(atoothache, anotcavity) * model.prior(anotcavity))
				/ model.prior(atoothache), DELTA_THRESHOLD);
		Assert.assertEquals(model.posterior(anotcavity, anottoothache),
				(model.posterior(anottoothache, anotcavity) * model
						.prior(anotcavity))
						/ model.prior(anottoothache), DELTA_THRESHOLD);
		//
		Assert.assertEquals(model.posterior(acavity, acatch), (model.posterior(
				acatch, acavity) * model.prior(acavity))
				/ model.prior(acatch), DELTA_THRESHOLD);
		Assert.assertEquals(model.posterior(acavity, anotcatch), (model
				.posterior(anotcatch, acavity) * model.prior(acavity))
				/ model.prior(anotcatch), DELTA_THRESHOLD);
		Assert.assertEquals(model.posterior(anotcavity, acatch), (model
				.posterior(acatch, anotcavity) * model.prior(anotcavity))
				/ model.prior(acatch), DELTA_THRESHOLD);
		Assert.assertEquals(model.posterior(anotcavity, anotcatch), (model
				.posterior(anotcatch, anotcavity) * model.prior(anotcavity))
				/ model.prior(anotcatch), DELTA_THRESHOLD);
	}
	...
	protected void test_MeningitisStiffNeckModel(ProbabilityModel model) {
		...
		// = (0.3 * 0.00002)/0.99
		// = 0.000006060606
		Assert.assertEquals(0.000006060606, model.posterior(ameningitis,
				anotstiffNeck), DELTA_THRESHOLD);
	}
	...
	protected void test_BurglaryAlarmModel(ProbabilityModel model) {
		...
		Assert.assertEquals(0.00059224, model.prior(aburglary, ajohnCalls,
				amaryCalls), DELTA_THRESHOLD);
		// P(Burglary = false | JohnCalls = true, MaryCalls = true) = 0.0014919
		Assert.assertEquals(0.00149185764899, model.prior(anotburglary,
				ajohnCalls, amaryCalls), DELTA_THRESHOLD);
	}
}
