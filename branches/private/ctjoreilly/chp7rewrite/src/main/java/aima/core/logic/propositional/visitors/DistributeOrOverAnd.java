...
public class DistributeOrOverAnd extends AbstractPLVisitor<Object> {
	...
	public Sentence visitBinarySentence(ComplexSentence s, Object arg) {
		...
		if (s.isOrSentence()) {
			Sentence s1 = s.getSimplerSentence(0);
			Sentence s2 = s.getSimplerSentence(1);
			if (s1.isAndSentence() || s2.isAndSentence()) {
				...
				// Ensure distributivity is applied to
				// contained simpler sentences.
				s1 = s1.accept(this, null);
				s2 = s2.accept(this, null);
