...
public class RouteCalculator {
	 ...
	 * two map nodes. It searches for way nodes in the vicinity of the two
	 * given nodes which comply with the specified way selection, searches
	 * for a suitable path, and adds the path as track to the provided
	 * <code>mapData</code>. The two factory methods can be used to
	...
	public void calculateRoute(List<MapNode> locs, MapDataStore mapData,
		...
		try {
			...
			MapNode toRNode = new Position(locs.get(locs.size()-1)).selectNearest(rNodes, wayFilter);
			
			Problem problem = createProblem
			(fromRNode, toRNode, mapData, wayFilter, ignoreOneways);
			Search search = new AStarSearch(new GraphSearch(), new OsmSldHeuristicFunction(toRNode));
