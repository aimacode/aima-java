...
/**
 * <p>Universal frame for building graphical agent applications. It provides
 * buttons for controlling the application and two panels and
 * a status bar to visualize agent, environment, and general simulation
 * state.</p> 
 * <p>To make the frame fit to your needs, you will at least have to add
 * some selectors and replace the dummy agent view pane by a view which
 * is capable to visualize your agent. The frame is configurable at run-time,
 * so subclassing will not always be necessary.</p> 
 * @author R. Lunde
 */
...
implements AgentAppModel.ModelChangedListener {
	 ...
	 * Each combobox has a name, which is used to access its selection
	 * state on software level and optionally a tool tip, which is shown
	 * to the user.
	 * @param tooltips Array of strings or null.
	 * 
	 */
	...
	/**
	 * Returns an object which represents the current selection state of
	 * all selectors.
	 */
	...
	/**
	 * Template method, replacing the agent view. The agent view
	 * is the panel to the left of the splitbar. It typically
	 * implements a 2D-visualization of the agent in its environment.
	 */
...
//		clearMenuItem.addActionListener(new FrameActionListener());
...
//		setJMenuBar(menuBar);
		...
		agentView = new AbstractAgentView() {
			public void updateView(AgentAppModel model) {} // dummy implementation
		};
	...
	/**
	 * Contains the names of all selectors and the indices of their
	 * selected items. Instances are used to communicate the selection
	 * state between the frame and the controller.
	 */
