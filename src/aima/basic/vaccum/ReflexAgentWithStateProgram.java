...
public abstract class ReflexAgentWithStateProgram extends AgentProgram {
	...
	public ReflexAgentWithStateProgram() {
		init();
		// Implementors of the init() method should have ensured the state and rules are setup
		assert(null != state);
		assert(null != rules);
	}
	...
	/**
	 * Realizations of this class should implement the init() method so that it calls
	 * the setState() and setRules() method.
	 */
	protected abstract void init();
	
	protected abstract ObjectWithDynamicAttributes updateState(ObjectWithDynamicAttributes envState, String anAction, Percept percept);
	
	protected Rule ruleMatch(ObjectWithDynamicAttributes envState, Set<Rule> rulesSet) {
		for (Rule r: rulesSet) {
