...
public class ReflexVaccumAgentWithState extends Agent {
	...
	public ReflexVaccumAgentWithState() {
		super(new ReflexAgentWithStateProgram() {
			...
			protected ObjectWithDynamicAttributes updateState(ObjectWithDynamicAttributes envState, String anAction, Percept percept) {
				envState.setAttribute("currentLocation", percept.getAttribute("location"));
				envState.setAttribute("currentStatus",   percept.getAttribute("status"));
				// Keep track of the status of the different locations
				envState.setAttribute((new StringBuffer("statusLocation").append(percept.getAttribute("location"))).toString(), percept.getAttribute("status"));
	...
	private static Set<Rule> getRuleSet() {
		...
		rules.add(new Rule(new ANDCondition(new EQUALCondition("statusLocationA", "Clean"), new EQUALCondition("statusLocationB", "Clean")), ReflexAgentWithStateProgram.NO_OP));
		rules.add(new Rule(new EQUALCondition("currentStatus",   "Dirty"), "Suck"));
		rules.add(new Rule(new EQUALCondition("currentLocation", "A"),     "Right"));
