...
public class MixedRadixInterval implements Iterable<int[]> {
    ...
    /**
     * Returns the value of the mixed radix interval given the specified array of
     * numerals.
     *
     ...
     *             of the specified numerals is greater than or equal to it's corresponding
     *             radix.
     */
    public BigInteger getValueFor(int[] numeralValues) {
        ...
        for (int i = numeralValues.length-1; i >= 0; i--) {
            if (numeralValues[i] < 0 || numeralValues[i] >= radices[i]) {
                throw new IllegalArgumentException("Radix value "+i+" is out of range for radix at this position, which is "+radices[i]);
            }
    ...
    public int[] getNumeralsFor(BigInteger value) {
        if (getMinPossibleValue().compareTo(value) > 0 || getMaxPossibleValue().compareTo(value) < 0) {
            throw new IllegalArgumentException("The value of "+value+" is outside of the possible values that can be represented by this interval's radix values ["+getMinPossibleValue()+", "+getMaxPossibleValue()+"]");
        }
    ...
    private class MixedRadixIntervalSpliterator implements Spliterator<int[]> {
        ...
        public Spliterator<int[]> trySplit() {
            ...
            if (BigInteger.ONE.compareTo(remainingSize) < 0) {
                ...
                MixedRadixInterval prefix = new MixedRadixInterval(radices,
                        getNumeralsFor(currentValue),
                        getNumeralsFor(splitPos));
                // Move this iterator to the end of the prefix, will advance past it when tryAdvance is called
                // and will never actually use the same value as the prefix does for its endpoint.
                current = getNumeralsFor(splitPos);
                // Update the estimated size and characteristics of the current spliterator
        ...
        private void updateCharacteristics() {
            ...
            if (MAX_LONG.compareTo(estSize) > 0) {
                // Can support the APIs reporting of size (i.e. restricted to long)
