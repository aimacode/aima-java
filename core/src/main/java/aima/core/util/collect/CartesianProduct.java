...
/**
 * An {@link java.lang.Iterable} over an <a href="http://en.wikipedia.org/wiki/Cartesian_product">n-fold Cartesian product</a>,
 * represented by a {@link java.util.List} of n dimensions, where each element is an n-length {@link java.util.List}.
 *
 * @param <T> the type of elements in the Cartesian Product.
 *
...
public class CartesianProduct<T> implements Iterable<T[]> {
    ...
    private T[][]              dimensions; // For access efficiency we will access the elements from a contiguous 2d array.
    // However, this will waste memory depending on the difference between the max
    // and min dimension lengths that can exist.
    ...
    public CartesianProduct(Class<T> elementBaseType, List<List<? extends T>> dimensions) {
        ...
        this.dimensions = (T[][]) Array.newInstance(elementBaseType, radices.length, IntStream.of(radices).max().getAsInt());
     ...
     *
     * @param elementBaseType the base type of the elements in the list (to permit construction of a type safe array).
     * @param dimension       the dimension to be raised to a given power.
