...
/**
 * <pre>
 * function GOAL-TESTED-QUEUED-FIRST-TREE-SEARCH(problem) returns a solution, or failure
 ...
 */
public class GoalTestedQueuedFirstTreeSearch<A, S> implements Search<A, S> {

	private NodeFactory<A, S> nodeFactory;
    private Supplier<FrontierQueueWithStateTracking<A, S>> frontierSupplier;
    
    public GoalTestedQueuedFirstTreeSearch(Supplier<FrontierQueueWithStateTracking<A, S>> frontierSupplier) {
    	this(new BasicNodeFactory<>(), frontierSupplier);
    }
    
    public GoalTestedQueuedFirstTreeSearch(NodeFactory<A, S> nodeFactory, Supplier<FrontierQueueWithStateTracking<A, S>> frontierSupplier) {
