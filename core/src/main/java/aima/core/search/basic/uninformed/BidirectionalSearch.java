...
public class BidirectionalSearch<A, S> implements SearchForActionsBidirectionallyFunction<A, S> {
    ...
    @Override
    public List<A> apply(BidirectionalProblem<A, S> problem) {
        Node<A, S> node = newRootNode(problem.initialState(), 0);
        if (problem.isGoalState(node.state())) {
            return solution(node);
        }
        Node<A, S> revNode = newRootNode(problem.getReverseProblem().initialState(), 0);
    ...
    public Node<A, S> pathExistsBidirectional(Queue<Node<A, S>> queue, Set<S> exploredFront, Set<S> exploredBack, BidirectionalProblem<A, S> problem) {
        if (!queue.isEmpty()) {
    ...
    public Node<A, S> newChildNode(BidirectionalProblem<A, S> problem, Node<A, S> node, A action) {
        return nodeFactory.newChildNodeForBidirectionalSearch(problem, node, action);
    }
