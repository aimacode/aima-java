 ...
 * 
 * <p>The second is a kd-tree (see {@link aimax.osm.data.impl.KDTree}).</p>
 * 
...
public interface MapDataStorage extends WayNodeProvider {
	...
	/**
	 * Returns a builder object, which receives map entities, for example from
	 * a reader, and builds up needed structures in the map for storing the
	 * entities (e.g. spatial indices).
	 */
	public MapContentBuilder getContentBuilder();
	
	/**
	 * Closes all open resources and should be called before
	...
	/**
	 * Returns the number of all maintained point of interests. Nodes
	...
	/**
	 * Returns all points of interest. Nodes are classified as POIs if they
	 * have a name or other attributes of interest.
	 */
	public List<MapNode> getPois(BoundingBox bb);
	
	/**
	 * Returns all nodes which are marked with the attribute place and
	 * whose name matches the specification.
	 */
	...
	/**
	 * Enables interested visitors to visit all entities within a certain
	 * area which are visible in the specified scale. 
	 */
	public void visitEntities(EntityVisitor visitor, BoundingBox bb, float scale);
	
	/**
	 * Returns a search engine for finding entities with specific attributes
	 * and names in the map.
	 * @return
	 */
