...
public class ParticleSet {
    ...
    //Use these two  to get the filtered set directly. This is the only method a user class needs to call.
    //The other methods are public only to be accessible to tests.
    
    public ParticleSet filter(String perception,Randomizer r) {	
	return generateParticleSetForPredictedState(r).perceptionUpdate(perception,r);	
    }
    
    public ParticleSet filter(String action, String perception, Randomizer r) {
	return generateParticleSetForPredictedState(action,r).perceptionUpdate(perception,r);
    }
	...
	for (String state : stateCount.keySet()){
	    result.setProbabilityOf(state,((double)stateCount.get(state)/particles.size()));
	}
	return result;
    }
	...
	for (Particle p:particles){
	    String newState =  hmm.transitionModel().getStateForProbability(p.getState(),action,randomizer.nextDouble());
	...
	for (Particle p:particles){
	    double particleWeight = hmm.sensorModel().get(p.getState(),perception);
