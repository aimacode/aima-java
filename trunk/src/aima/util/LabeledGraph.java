...
/**
 * Represents a directed labeled graph. Vertices are represented by
 * their unique labels and labeled edges by means of nested hashtables.
 * Variant of class {@code aima.util.Table}. This version is more dynamic,
 * it requires no initialization and can add new items whenever needed.
 * @author R. Lunde
 */
public class LabeledGraph<VertexLabelType, EdgeLabelType> {
	
	/**
	 * Lookup for edge label information. Contains an entry for every
	 * vertex label.
	 */
	...
	/**
	 * Adds a directed labeled edge to the graph. The end points of the
	 * edge are specified by vertex labels. New vertices are automatically
	 * identified and added to the graph.
	 */
	public void set(VertexLabelType from, VertexLabelType to, EdgeLabelType el) {
		Hashtable<VertexLabelType, EdgeLabelType> localEdgeLookup = 
			checkForNewVertex(from);
	...
	/**
	 * Returns the label of the edge between the specified vertices and
	 * null if there is no edge between them.
	 */
	...
	/**
	 * Returns the labels of those vertices which can be obtained by
